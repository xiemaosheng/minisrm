...
function(win) {
    ...
    Dates.run = function(options) {
        ...
        if (even && target.tagName) {
            ...
            Dates.each((elem.length | 0) > 0 ? elem: [elem],
            function(ii, that) {
    ...
    Dates.checkVoid = function(YY, MM, DD) {
        ...
        if (YY < Dates.mins[0]) {
            ...
            if (YY > Dates.maxs[0]) {
                ...
                if (YY >= Dates.mins[0] && YY <= Dates.maxs[0]) {
                    if (YY == Dates.mins[0]) {
                        if (MM < Dates.mins[1]) {
                            back = ["m"]
                        } else {
    ...
    Dates.timeVoid = function(times, index) {
        if (Dates.ymd[1] + 1 == Dates.mins[1] && Dates.ymd[2] == Dates.mins[2]) {
            if (index === 0 && (times < Dates.mins[3])) {
                ...
                if (index === 1 && times < Dates.mins[4]) {
                    return 1
                } else {
                    if (index === 2 && times < Dates.mins[5]) {
                        return 1
                    }
                }
            }
            ...
            if (Dates.ymd[1] + 1 == Dates.maxs[1] && Dates.ymd[2] == Dates.maxs[2]) {
                if (index === 0 && times > Dates.maxs[3]) {
                    ...
                    if (index === 1 && times > Dates.maxs[4]) {
                        return 1
                    } else {
                        if (index === 2 && times > Dates.maxs[5]) {
                            return 1
                        }
                    }
                }
            ...
            if (!exp.test(value)) {
                ...
                if (isvoid[0]) {
                    ...
                    if (arr[2] < 1) {
                        ...
                        if (arr[2] > Dates.months[(arr[1] | 0) - 1]) {
                            arr[2] = 31;
                            isvoid.auto = 1
                        } else {
                            if (arr[2].length < 2) {
                    ...
                    if (isvoid.auto) {
                        ...
                        if (isvoid.value !== Dates.elem[as.elemv]) {
    ...
    Dates.viewDate = function(Y, M, D) {
        ...
        function(i, elem) {
            ...
            if (i < log.FDay) {
                ...
                if (i >= log.FDay && i < log.FDay + Dates.months[log.ymd[1]]) {
        ...
        log.times = [Dates.inymd[Dates.elemIndexMap.hour] | 0 || 0, Dates.inymd[Dates.elemIndexMap.minute] | 0 || 0, Dates.inymd[Dates.elemIndexMap.second] | 0 || 0];
        Dates.each(new Array(3),
        function(i) {
    ...
    Dates.orien = function(obj, pos) {
        ...
        var objWidth = parseFloat(obj.clientWidth);
        var rectLeft = parseFloat(rect.left);
        var bodyWidth = parseFloat(document.body.clientWidth);
        var left = objWidth + rectLeft > bodyWidth ? (bodyWidth - objWidth - 40) : rectLeft;
    ...
    Dates.viewtb = (function() {
        ...
        Dates.each(new Array(6),
        function(i) {
            view.push([]);
            tr = table.insertRow(0);
            Dates.each(new Array(7),
            function(j) {
                view[i][j] = 0;
                i === 0 && log.creath(j);
                tr.insertCell(j)
    ...
    Dates.view = function(elem, options) {
        ...
        if (!Dates.box) {
            ...
            div.id = as[0];
            div.className = as[0];
            div.style.cssText = "position: absolute;top: 0px; left: 15px;";
    ...
    Dates.reshow = function() {
        Dates.each(Dates.query("#" + as[0] + " .laydate_show"),
        function(i, elem) {
    ...
    Dates.events = function() {
        ...
        Dates.each(S(log.box + " .laydate_ym"),
        function(i, elem) {
            Dates.on(elem, "click",
            function(ev) {
                Dates.stopmp(ev).reshow();
        ...
        Dates.each(S("#laydate_YY .laydate_tab"),
        function(i, elem) {
            Dates.on(elem, "click",
            function(ev) {
                Dates.stopmp(ev);
        ...
        Dates.each(S("#laydate_MM .laydate_tab"),
        function(i, elem) {
            Dates.on(elem, "click",
            function(ev) {
                Dates.stopmp(ev).reshow();
        ...
        Dates.msg = function(i, title) {
            ...
            if (typeof i === "string") {
                ...
                if (!log.hmsarr[i]) {
                    ...
                    Dates.each(new Array(i === 0 ? 24 : 60),
                    function(i) {
                        str += "<span>" + i + "</span>"
                    });
                    str += "</div>";
    ...
    laydate.reset = function() { (Dates.box && Dates.elem) && Dates.follow(Dates.box)
    };
    laydate.now = function(timestamp, format) {
        var De = new Date((timestamp | 0) ?
        function(tamp) {
